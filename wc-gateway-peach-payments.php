...
add_action( 'plugins_loaded', 'wc_peach_payments_init', 0 );
...
if ( in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) {
	...
	function wc_peach_payments_init() {
		...
		load_plugin_textdomain( 'wc-gateway-peach-payments', false, trailingslashit( dirname( plugin_basename( __FILE__ ) ) ) );
		...
		add_filter( 'woocommerce_payment_gateways', 'wc_peach_payments_add_gateway' );
		...
		function woocommerce_peach_handle_delete_card() {
			...
			if ( ! is_user_logged_in() && ! wp_verify_nonce( $_POST['_wpnonce'], 'peach_del_card' ) ) {
				wp_die( esc_html_e( 'Unable to verify deletion, please try again', 'wc-gateway-peach-payments' ) );
			}
			...
			wc_add_notice( esc_html_e( 'Card deleted.', 'wc-gateway-peach-payments' ), 'success' );
		...
		function woocommerce_peach_saved_cards() {
			...
			if ( ! $credit_cards )
				return;
			?>
				<h2 id="saved-cards" style="margin-top:40px;"><?php esc_html_e( 'Saved cards', 'wc-gateway-peach-payments' ); ?></h2>
				<table class="shop_table">
					<thead>
						<tr>
							<th><?php esc_html_e( 'Brand', 'wc-gateway-peach-payments' ); ?></th>
							<th><?php esc_html_e( 'Card Number', 'wc-gateway-peach-payments' ); ?></th>
							<th><?php esc_html_e( 'Expires', 'wc-gateway-peach-payments' ); ?></th>
							<th></th>
						</tr>
					</thead>
					<tbody>
						...
						<tr>
							...
							<td>
								<form action="" method="POST">
									...
									<input type="submit" class="button" value="<?php esc_html_e( 'Delete card', 'wc-gateway-peach-payments' ); ?>">
								</form>
							</td>
						</tr>
						<?php endforeach; ?>
 ...
 * @since 1.0.0
 */
function wc_peach_payments_add_gateway( $methods ) {
	if ( class_exists( 'WC_Subscriptions_Order' ) ) {
	...
	return $methods;
} // End wc_peach_payments_add_gateway()
